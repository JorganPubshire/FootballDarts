...
from dart_football.engine.events import ChooseKickOrReceive, CoinTossWinner, KickoffKick
...
def test_correct_kickoff(rules, rules_path: Path, tmp_path: Path) -> None:
    ...
    # PDF kickoff: segments 1–5 → receiving team ball at own 40
    assert line_right == 40
    assert sess.records[-1].supersedes_seq == 3
